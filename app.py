...
def calculate_reference_event_change(dataframe):
    ...
    grouped = (
        quant.groupby(
            ["Year", "State", "City", "Local", "WWTP", "Event", "Substance", "Period"],
            as_index=False
        )["PNML_mg_day_1000inh"]
        .mean()
    )
    ...
    pivot = grouped.pivot_table(
        index=["Year", "State", "City", "Local", "WWTP", "Event", "Substance"],
        columns="Period",
    ...
    pivot["Increase_vs_Reference_%"] = pd.NA
    mask_ref = pivot["Reference"].notna() & pivot["During"].notna() & (pivot["Reference"] != 0)
    pivot.loc[mask_ref, "Increase_vs_Reference_%"] = (
        (pivot.loc[mask_ref, "During"] - pivot.loc[mask_ref, "Reference"]) /
        pivot.loc[mask_ref, "Reference"]
    ) * 100

    pivot["Increase_vs_Before_%"] = pd.NA
    mask_before = pivot["Before"].notna() & pivot["During"].notna() & (pivot["Before"] != 0)
    pivot.loc[mask_before, "Increase_vs_Before_%"] = (
        (pivot.loc[mask_before, "During"] - pivot.loc[mask_before, "Before"]) /
        pivot.loc[mask_before, "Before"]
    ) * 100

    pivot["Recovery_After_vs_During_%"] = pd.NA
    mask_after = pivot["After"].notna() & pivot["During"].notna() & (pivot["During"] != 0)
    pivot.loc[mask_after, "Recovery_After_vs_During_%"] = (
        (pivot.loc[mask_after, "After"] - pivot.loc[mask_after, "During"]) /
        pivot.loc[mask_after, "During"]
    ) * 100
...
df_year_sidebar = df[df["Year"].isin(year_filter)] if year_filter else df.copy()

state_filter = st.sidebar.multiselect(
    "State",
    unique_sorted(df_year_sidebar, "State"),
    default=unique_sorted(df_year_sidebar, "State")
)

df_state_sidebar = df_year_sidebar[df_year_sidebar["State"].isin(state_filter)] if state_filter else df_year_sidebar.copy()

local_filter = st.sidebar.multiselect(
    "Local",
    unique_sorted(df_state_sidebar, "Local"),
    default=unique_sorted(df_state_sidebar, "Local")
)

df_local_sidebar = df_state_sidebar[df_state_sidebar["Local"].isin(local_filter)] if local_filter else df_state_sidebar.copy()

wwtp_filter = st.sidebar.multiselect(
    "WWTP",
    unique_sorted(df_local_sidebar, "WWTP"),
    default=unique_sorted(df_local_sidebar, "WWTP")
)

df_wwtp_sidebar = df_local_sidebar[df_local_sidebar["WWTP"].isin(wwtp_filter)] if wwtp_filter else df_local_sidebar.copy()

event_filter = st.sidebar.multiselect(
    "Event",
    unique_sorted(df_wwtp_sidebar, "Event"),
    default=unique_sorted(df_wwtp_sidebar, "Event")
)

df_event_sidebar = df_wwtp_sidebar[df_wwtp_sidebar["Event"].isin(event_filter)] if event_filter else df_wwtp_sidebar.copy()

period_filter = st.sidebar.multiselect(
    "Period",
    unique_sorted(df_event_sidebar, "Period"),
    default=unique_sorted(df_event_sidebar, "Period")
)

st.sidebar.markdown("---")
st.sidebar.markdown("### Analytical Filters")

df_period_sidebar = df_event_sidebar[df_event_sidebar["Period"].isin(period_filter)] if period_filter else df_event_sidebar.copy()

substance_filter = st.sidebar.multiselect(
    "Substance",
    unique_sorted(df_period_sidebar, "Substance")
)

drug_class_filter = st.sidebar.multiselect(
    "Drug Class",
    unique_sorted(df_period_sidebar, "Drug_Class")
)

platform_filter = st.sidebar.multiselect(
    "Analytical Platform",
    unique_sorted(df_period_sidebar, "Analytical_Platform")
)

analysis_filter = st.sidebar.multiselect(
    "Analysis Type",
    unique_sorted(df_period_sidebar, "Analysis_Type")
)

detection_filter = st.sidebar.multiselect(
    "Detection",
    unique_sorted(df_period_sidebar, "Detection")
)
...
with tab_compare:
    ...
    st.info(
        "This section compares PNML values from the Reference period with the selected event period "
        "for the same Year, Local, WWTP and Substance."
    )
    ...
    if len(comparison) > 0:
        available_phases = [
            phase for phase in ["Reference", "Before", "During", "After"]
            if phase in comparison.columns and comparison[phase].notna().any()
        ]

        default_phases = [phase for phase in ["Reference", "During"] if phase in available_phases]
        if not default_phases:
            default_phases = available_phases

        phases_to_show = st.multiselect(
            "Periods to compare",
            available_phases,
        ...
        comparison_long = comparison.melt(
            id_vars=["Year", "State", "City", "Local", "WWTP", "Event", "Substance"],
            value_vars=phases_to_show,
            var_name="Comparison_Period",
            value_name="PNML_mg_day_1000inh"
        ).dropna(subset=["PNML_mg_day_1000inh"])

        comparison_long = make_plot_df(
            comparison_long,
            y_columns=["PNML_mg_day_1000inh"]
        )

        if len(comparison_long) > 0:
            fig_reference_bars = px.bar(
                comparison_long,
                x="Comparison_Period",
                ...
                facet_col="Event",
                hover_data=[
                    "Year", "State", "City", "Local", "WWTP",
                    "Event", "Substance", "Comparison_Period"
                ],
                title="Reference vs Event Periods by Substance",
                labels={
            ...
            fig_reference_bars.update_layout(
                legend_title_text="Substance",
                yaxis_title="PNML (mg/day/1000 inhabitants)"
            )

            st.plotly_chart(fig_reference_bars, use_container_width=True)

            trend_long = comparison_long.copy()
            period_order = {"Reference": 0, "Before": 1, "During": 2, "After": 3}
            trend_long["Period_Order"] = trend_long["Comparison_Period"].map(period_order)
            trend_long = trend_long.sort_values(
                ["Year", "State", "City", "WWTP", "Event", "Substance", "Period_Order"]
            )

            if trend_long["Comparison_Period"].nunique() > 1:
                fig_reference_line = px.line(
                    trend_long,
                    x="Comparison_Period",
                    y="PNML_mg_day_1000inh",
                    color="Substance",
                    ...
                    title="Reference → Event Period Profile",
                    labels={
                        "Comparison_Period": "Period",
                        "PNML_mg_day_1000inh": "PNML (mg/day/1000 inhabitants)"
                    }
                )
                st.plotly_chart(fig_reference_line, use_container_width=True)

            complete_cols = [
                "Year", "State", "City", "Local", "WWTP", "Event", "Substance",
                "Reference", "Before", "During", "After",
                "Increase_vs_Reference_%", "Increase_vs_Before_%", "Recovery_After_vs_During_%"
            ]

            complete_cols = [col for col in complete_cols if col in comparison.columns]

            st.markdown("### Complete Comparison Table")
            st.dataframe(
                comparison[complete_cols],
                use_container_width=True
            )

        else:
            empty_message(
                "No Reference/Event comparison data available for the selected filters. "
                "Check whether Reference and at least one event period are selected."
            )
