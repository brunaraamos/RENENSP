...
def calculate_reference_event_change(dataframe):
    """Build a wide Reference vs event comparison table.

    The comparison is made by Year + Local + WWTP + Event + Substance.
    If Reference is stored with the same Event name, it is used directly.
    If Reference is stored as a general baseline for the WWTP/year/substance,
    ...
    keys_event = ["Year", "State", "City", "Local", "WWTP", "Event", "Substance"]
    keys_reference = ["Year", "State", "City", "Local", "WWTP", "Substance"]

    grouped = (
        quant.groupby(keys_event + ["Period"], as_index=False)["PNML_mg_day_1000inh"]
        .mean()
    )
    ...
    reference_general = (
        quant[quant["Period"].eq("Reference")]
        .groupby(keys_reference, as_index=False)["PNML_mg_day_1000inh"]
        .mean()
        .rename(columns={"PNML_mg_day_1000inh": "Reference_Fallback"})
    )

    pivot = pivot.merge(reference_general, on=keys_reference, how="left")
    pivot["Reference"] = pivot["Reference"].fillna(pivot["Reference_Fallback"])
    pivot = pivot.drop(columns=["Reference_Fallback"])
...
def build_reference_event_long(comparison, event_period):
    """Convert the comparison table to long format with two bars:
    Reference and selected event period.
    """
    if len(comparison) == 0 or event_period not in comparison.columns:
        return pd.DataFrame()

    needed_cols = [
        "Year", "State", "City", "Local", "WWTP", "Event", "Substance",
        "Reference", event_period
    ]

    temp = comparison[needed_cols].copy()
    temp = temp.dropna(subset=["Reference", event_period])
    ...
    long_df = temp.melt(
        ...
        value_vars=["Reference", event_period],
        var_name="Comparison_Period",
        value_name="PNML_mg_day_1000inh"
    )

    long_df["Comparison_Period"] = long_df["Comparison_Period"].replace(
        {event_period: f"Event: {event_period}"}
    )
...
with tab_compare:
    ...
    st.info(
        "This section compares the baseline Reference with an event period "
        "for the same Year, Local, WWTP, Event and Substance. "
        "The main chart shows two grouped bars: Reference vs the selected event period."
    )

    if "Reference" not in selected_periods:
        st.warning(
            "For this comparison, keep Reference selected in the Period filter."
        )
    ...
    if len(comparison) > 0:
        event_period_options = [
            period for period in ["During", "Before", "After"]
            if period in comparison.columns and comparison[period].notna().any()
        ]

        if len(event_period_options) == 0:
            empty_message("No event period with PNML data is available for comparison.")
        else:
            selected_event_period = st.radio(
                "Select event period to compare with Reference",
                event_period_options,
                index=event_period_options.index("During") if "During" in event_period_options else 0,
                horizontal=True
            )

            comparison_long = build_reference_event_long(
                comparison,
                selected_event_period
            )

            if len(comparison_long) > 0:
                st.markdown(
                    f"### Reference vs {selected_event_period} PNML"
                )
                ...
                fig_ref_event = px.bar(
                    ...
                    title=f"Reference vs {selected_event_period} by substance, event and WWTP",
                    labels={
                ...
                st.markdown("### Temporal Profile: Reference → Before → During → After")
                ...
                timeline_long = timeline_long.dropna(subset=["PNML_mg_day_1000inh"])

                period_order = ["Reference", "Before", "During", "After"]
                timeline_long["Period"] = pd.Categorical(
                    timeline_long["Period"],
                    categories=period_order,
                    ordered=True
                )
                timeline_long = timeline_long.sort_values("Period")
                ...
                fig_timeline = px.line(
                    ...
                    markers=True,
                    facet_col="Event",
                    hover_data=["Year", "State", "City", "Local", "WWTP", "Event"],
                    title="Temporal profile by period",
                    labels={
                        "PNML_mg_day_1000inh": "PNML (mg/day/1000 inhabitants)",
                        "Period": "Period"
                    }
                ...
                if len(comparison_plot) > 0:
                    st.markdown("### Increase from Reference to During")
                    ...
                    fig_increase_ref = px.bar(
                        ...
                        labels={
                            "Increase_vs_Reference_%": "Increase vs Reference (%)"
                        }
                    )
                    ...
                    st.dataframe(
                        top_increase[
                            [
                                "Year", "State", "City", "WWTP", "Event", "Substance",
                                "Reference", "Before", "During", "After",
                                "Increase_vs_Reference_%", "Increase_vs_Before_%"
                            ]
                        ],
                ...
                empty_message(
                    f"Reference and {selected_event_period} were not both available "
